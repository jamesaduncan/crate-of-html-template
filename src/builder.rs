...
impl HtmlTemplateBuilder {
    ...
    pub fn add_handler<S: Into<String>>(
        mut self,
        tag_name: S,
    ...
    pub fn build(self) -> Result<HtmlTemplate> {
        ...
        let source = self.source.ok_or_else(|| {
            Error::parse_static(
                "No template source provided. Use from_str(), from_element(), or from_file()",
            )
        })?;
        ...
        let template = if let Some(cache) = self.custom_cache {
            HtmlTemplate::from_str_with_cache(&html, self.selector.as_deref(), self.config, &cache)?
        } else {
            HtmlTemplate::from_str_with_config(&html, self.selector.as_deref(), self.config)?
        };
...
impl HtmlTemplateBuilder {
    ...
    pub fn quick_template_with_selector(html: &str, selector: &str) -> Result<HtmlTemplate> {
        Self::new().from_str(html).with_selector(selector).build()
    }
    ...
    pub fn performance_template(html: &str, selector: Option<&str>) -> Result<HtmlTemplate> {
        let mut builder = Self::new().from_str(html).aggressive_performance();
    ...
    pub fn development_template(html: &str, selector: Option<&str>) -> Result<HtmlTemplate> {
        let mut builder = Self::new().from_str(html).no_caching();
    ...
    pub fn from_template_file<P: AsRef<Path>>(path: P) -> Result<HtmlTemplate> {
        Self::new().from_file(path).build()
    }

    /// Create a template from a file with selector and performance optimizations
    pub fn from_template_file_optimized<P: AsRef<Path>>(
        path: P,
        selector: Option<&str>,
    ) -> Result<HtmlTemplate> {
        let mut builder = Self::new().from_file(path).aggressive_performance();
...
impl<'a> RenderBuilder<'a> {
    ...
    pub fn with_handler<S: Into<String>>(
        mut self,
        tag_name: S,
        handler: Box<dyn ElementHandler>,
    ) -> Self {
    ...
    pub fn render_with_metadata(
        self,
        data: &dyn crate::value::RenderValue,
    ) -> Result<RenderResult> {
...
mod tests {
    ...
    fn test_builder_no_source_error() {
        let result = HtmlTemplateBuilder::new().with_selector("div").build();

        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("No template source provided"));
    }
