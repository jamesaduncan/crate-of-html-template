...
static ARRAY_INDEX_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"(\w+)\[(\d+)\]").expect("Invalid array index regex"));
...
impl Compiler {
    ...
    fn optimize_template(template: &mut CompiledTemplate) -> Result<()> {
        ...
        template.elements.sort_by(|a, b| {
            Self::selector_specificity(&a.selector).cmp(&Self::selector_specificity(&b.selector))
        });
