...
impl<'a> AsyncStreamingRenderer<'a> {
    ...
    where
        S: futures::Stream<Item = std::result::Result<Box<dyn RenderValue>, E>> + std::marker::Unpin,
        E: Into<Error>,
    {
    ...
    pub async fn process_async_chunks<S, E, F>(
        &self,
        mut stream: S,
        mut callback: F,
    ) -> Result<()>
...
mod tests {
    ...
    fn test_streaming_result() {
        ...
        let items: Vec<Box<dyn RenderValue>> = data.iter().map(|d| Box::new(d.clone()) as Box<dyn RenderValue>).collect();
    ...
    fn test_chunked_processing() {
        ...
        streaming_result.for_each_chunk(|chunk| {
            chunk_count += 1;
