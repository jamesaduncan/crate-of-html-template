...
impl HandlerRegistry {
    ...
    pub fn register(&mut self, tag_name: &str, handler: Box<dyn ElementHandler>) {
        ...
        let tag_handlers = self
            .handlers
            .entry(tag_name.to_lowercase())
            .or_insert_with(Vec::new);

        // Insert in priority order (highest first)
        let insert_pos = tag_handlers
            .iter()
            .position(|(p, _)| *p < priority)
            .unwrap_or(tag_handlers.len());
    ...
    pub fn to_hashmap(&self) -> std::collections::HashMap<String, Box<dyn ElementHandler>> {
        ...
        for (_tag, handlers) in &self.handlers {
            if let Some((_, _handler)) = handlers.first() {
...
impl ElementHandler for LoggingHandler {
    ...
    fn handle(&self, element: &Selection, value: &dyn RenderValue) -> Result<()> {
        ...
        eprintln!(
            "Processing <{}> with itemprop='{}', value={:?}",
            tag_name,
            itemprop,
            value.get_property(&[])
        );
...
mod tests {
    ...
    fn test_select_handler() {
        ...
        assert_eq!(
            option_nodes[1].attr("selected").as_deref(),
            Some("selected")
        );
