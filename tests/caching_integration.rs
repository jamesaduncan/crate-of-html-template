...
fn test_cache_eviction_and_statistics() {
    ...
    let _template1 =
        HtmlTemplate::from_str_with_cache(html1, None, config.clone(), &cache).unwrap();
    let _template2 =
        HtmlTemplate::from_str_with_cache(html2, None, config.clone(), &cache).unwrap();
    let _template3 =
        HtmlTemplate::from_str_with_cache(html3, None, config.clone(), &cache).unwrap(); // Should evict template1
...
fn test_cache_hit_miss_statistics() {
    ...
    let _template1 = cache
        .get_or_compile_template(&key, || crate::compiler::Compiler::compile(html, None))
        .unwrap();

    // Second access should be a hit
    let _template2 = cache
        .get_or_compile_template(&key, || {
