...
use html_template::{
    CacheConfig, CacheMode, HtmlTemplate, HtmlTemplateBuilder, StreamingRenderer, TemplateCache,
    TemplateConfig,
};
use serde_json::json;
...
fn template_caching_performance() -> html_template::Result<()> {
    ...
    println!(
        "Speedup:         {:.2}x",
        no_cache_duration.as_secs_f64() / cached_duration.as_secs_f64()
    );
    println!();
    ...
    println!(
        "Additional speedup: {:.2}x",
        cached_duration.as_secs_f64() / reuse_duration.as_secs_f64()
    );
    println!("\n" + &"=".repeat(60) + "\n");
...
fn streaming_performance() -> html_template::Result<()> {
    ...
    println!(
        "Memory efficiency: Constant memory usage vs {} KB peak",
...
fn configuration_tuning() -> html_template::Result<()> {
    ...
    println!(
        "Production speedup: {:.2}x",
        dev_duration.as_secs_f64() / prod_duration.as_secs_f64()
    );
