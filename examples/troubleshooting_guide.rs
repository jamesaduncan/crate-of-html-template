...
use html_template::{HtmlTemplate, HtmlTemplateBuilder, TemplateConfig, CacheMode, Error};
use serde_json::json;
...
fn performance_issues() -> html_template::Result<()> {
    ...
    for i in 0..3 {
        let _template = HtmlTemplate::from_str_with_config(html, Some("div"), no_cache_config.clone())?;
        println!("  Template {} compiled", i + 1);
    }
    
    println!("With caching: Template compiled once, reused from cache");
    for i in 0..3 {
        let _template = HtmlTemplate::from_str_with_config(html, Some("div"), cached_config.clone())?;
